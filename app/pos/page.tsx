...
export default function PosTerminal() {
  ...
  const handleTransaction = async (e: React.FormEvent) => {
      ...
      setTimeout(() => { setSuccessMsg(''); setStep(2); setCustomer(null); setSearchQuery(''); setNewCustomerName(''); setAmountEuro('') }, 3000)
  }

  const handleScan = (results: any[]) => {
      if (results && results.length > 0) {
          const code = results[0].rawValue;
          setSearchQuery(code); setShowScanner(false); handleFindCustomer(undefined, code);
      }
  }
                       ...
                       <div className="bg-white p-6 rounded-3xl shadow-xl animate-in slide-in-from-bottom">
                           {showScanner ? (
                               ...
                               <>
                                   ...
                                   <button onClick={() => setShowScanner(true)} className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold mb-4 flex items-center justify-center gap-2 shadow-lg">
                                       <Camera size={20}/> SCANSIONA QR
                                   </button>

                                   <div className="flex items-center gap-2 my-4">
                                       <div className="h-px bg-gray-200 flex-1"></div>
                                       <span className="text-xs text-gray-400 font-bold uppercase">Oppure Manuale</span>
                                       <div className="h-px bg-gray-200 flex-1"></div>
                                   </div>

                                   <div className="space-y-3">
                                       <input type="text" placeholder="Nome Cliente (Solo se nuovo)" className="w-full p-4 bg-gray-50 border-2 border-gray-100 rounded-xl outline-none focus:border-[#00665E]" value={newCustomerName} onChange={e => setNewCustomerName(e.target.value)} />
                       ...
                       <div className="bg-white p-6 rounded-3xl shadow-xl animate-in slide-in-from-right">
                           ...
                           <form onSubmit={handleTransaction}>
                               ...
                               <div className="grid grid-cols-2 gap-3 mt-4">
                                   <button type="button" onClick={() => {setStep(2); setAmountEuro(''); setNewCustomerName('');}} className="py-4 text-gray-500 font-bold hover:bg-gray-50 rounded-xl">Annulla</button>

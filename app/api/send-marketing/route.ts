...
export async function POST(request: Request) {
  try {
    ...
    // 1. VERIFICA UTENTE REALE
    const { data: { user }, error: authError } = await supabase.auth.getUser(token);
    if (authError || !user) return NextResponse.json({ error: 'Utente non autorizzato' }, { status: 401 });

    const { subject, content, channel, recipients, campaign_id, catalog_products, custom_product } = await request.json();

    // 2. RECUPERA INFO AZIENDA DAL DATABASE (Non ci fidiamo del client)
    const { data: profile } = await supabase.from('profiles').select('company_name, logo_url').eq('id', user.id).single();
    const finalCompanyName = profile?.company_name || 'Comunicazione Aziendale';
    const finalCompanyLogo = profile?.logo_url || '';
    ...
    // Gestione canali non-email (Placeholder per implementazione futura)
    if (channel && channel !== 'email') {
        // Qui andrebbero le integrazioni con Twilio (SMS/WhatsApp) o Meta API
        console.log(`[Omnichannel] Invio richiesto su canale: ${channel} per ${recipients.length} contatti.`);
        // Per ora simuliamo successo o inviamo comunque mail se richiesto, 
        // ma tecnicamente dovremmo integrare i singoli provider.
        // Restituiamo successo simulato per testare il flusso di pagamento/pricing nel frontend.
      ...
      <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin: 0 auto;">
          <tr>
              <td style="text-align: center;">
                  ${finalCompanyLogo && finalCompanyLogo.trim() !== '' && finalCompanyLogo.startsWith('http') 
                      ? `<img src="${finalCompanyLogo}" alt="${finalCompanyName}" style="max-height: 80px; max-width: 250px; display: block; margin: 0 auto;" />` 
                      : `
